        ...
        <v-row class="ma-0" justify="center" align="center">
          <v-col cols="12" sm="7" md="5" lg="4" xl="3" class="pa-0">
            <tips-page-card
              ...
              :title="$vuetify.lang.t('$vuetify.emptyTip.title.blackHoleTag')"
            ></tips-page-card>
          </v-col>
        </v-row>
      </tag-list-container-normal>
...
export default {
  ...
  components: {
    ...
    "tag-list-container-normal": () =>
      import("./components/TagListContainerNormal")
  },
  props: {
    page: {
      type: [String, Number],
  ...
  methods: {
    ...
    changePage(page) {
      ...
      if (this.mode === this.$enum.ListMode.WATERFALL.key) {
        ...
        this.$router.push(`/blackHole/tag/${page}`)
      }
    },
    async paging(pageIndex) {
      ...
      this.loading = true
      const result = await tagBlackHoleService.paging(this.pageable)
      this.loading = false
      await this.$resultNotify(result)
      this.currPage = result.data
    },
    block({ detail, state }) {
      let row = this.currPage.content.find((it) => it.name === detail.name)
      row.state = state
    }
  }
}
